...
/**
 * 【产量年报表:YQCJ_PP_YY】的Dao操作类
 * 
 */
public class PmcPpYyDao extends DAO {
    /**
     * 查询所有【产量年报表:YQCJ_PP_YY】信息
     * 
     * @param con
     * @return
     * @throws Exception
    ...
    public static List queryPmcPpYy(Connection con, Date yyyy,String banci) throws Exception {
        ...
        logger.debug("油漆车间YQCJ_PP_YY查询所有【产量年报表:YQCJ_PP_YY】信息:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("YQCJ_PP_YY", con);
    }

    public static int countPmcPpYy(Connection con, Date yyyy) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select count(*) as TOTAL_NUM  from YQCJ_PP_YY ");
        cqExp.where();
        ...
        logger.debug("油漆车间YQCJ_PP_YY查询所有【产量年报表:YQCJ_PP_YY】信息:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("YQCJ_PP_YY", con).get("TOTAL_NUM")));
    }
}
