...
/**
 * 【意见反馈表:YQCJ_VIEW_RECORD】的Dao操作类
 * 
 */
public class PmcViewRecordDao extends DAO {
    /**
     * 查询所有【意见反馈表:YQCJ_VIEW_RECORD】信息
     * 
     * @param con
     * @return
     * @throws Exception
    ...
    public static List queryPmcViewRecord(Connection con, Date startPlanDate, Date endPlanDate) throws Exception {
        ...
        cqExp.select("select *");
        cqExp.select(" from YQCJ_VIEW_RECORD");
        cqExp.where();
        ...
        logger.debug("油漆车间YQCJ_VIEW_RECORD查询所有【信息反馈:YQCJ_VIEW_RECORD】信息:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("YQCJ_VIEW_RECORD", con);
    }
    ...
    public static DynaBeanMap getPmcViewRecordByPk(Connection con, java.lang.Integer id) throws Exception {
        ...
        sql.append("select * from YQCJ_VIEW_RECORD where  ID=?");
        CQuery query = CQueryFactoryTool.createFactory().createCQuery();
        query.setCommand(sql.toString());
        query.setInt(1, id);
        dynaBeanMap = query.getDynaBeanMap("YQCJ_VIEW_RECORD", con);
        return dynaBeanMap;
    }
    ...
    public static int countPmcViewRecord(Connection con, Date startPlanDate, Date endPlanDate) throws Exception {
        ...
        logger.debug("油漆车间YQCJ_VIEW_RECORD统计所有【信息反馈:YQCJ_VIEW_RECORD】信息:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("YQCJ_VIEW_RECORD", con).get("TOTAL_NUM")));
    }
}
