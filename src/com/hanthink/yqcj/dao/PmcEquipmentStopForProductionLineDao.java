...
public class PmcEquipmentStopForProductionLineDao extends DAO {
     ...
     * @throws Exception
     */
    public static List queryPmcEquipmentStopAgen(Connection con, Date startDate, Date endDate,String banci) throws Exception {
        CQExp cqExp = CQExp.instance();
        
        cqExp.select("SELECT PRODUCTIONLINE AS PRODUCTIONLINE,PRODUCTIONLINENAME AS PRODUCTIONLINENAME,BANCI AS BANCI,");
        cqExp.select("SUM(STOPTIME)/60 as STOPTIME,SUM(STOPCOUNT) as STOPCOUNT,cast(AVG(CYCLETIME) as decimal(18,2)) AS CYCLETIME FROM YQCJ_EQUIPMENT_STOPLINE");
        cqExp.where();
        ...
        logger.debug(" 油漆车间查询瓶颈工段信息:  " + cqExp.getSql());
    ...
    public static List queryPmcEquipmentStopCount(Connection con, Date startDate, Date endDate, java.lang.String pdLine) throws Exception {
        ...
        cqExp.select("EventData4 AS STATION,SUM(EventData1)/60 as STOPTIME,COUNT(1) as STOPCOUNT FROM YQCJ_TABSTOPSTA");
        cqExp.where();
        ...
        logger.debug(" 油漆车间TABSTOPSTA查询所有【工段停线报表:TABSTOPSTA】信息:  " + cqExp.getSql());
    ...
    public static int countPmcEquipmentStop(Connection con, Date ppdate, java.lang.String pdLine) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select count(*) as TOTAL_NUM from YQCJ_EQUIPMENT_STOP ");
        cqExp.where();
        ...
        logger.debug(" 油漆车间YQCJ_EQUIPMENT_STOP统计所有【工段停线报表:YQCJ_EQUIPMENT_STOP】信息:  " + cqExp.getSql());
    ...
    public static List queryPmcEquipmentStopMonth(Connection con, Date ppdate, java.lang.String pdLine) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select eventdata15 as PRODUCTIONLINE,eventdata16 as PRODUCTIONLINENAME,eventdata1 as STATION,convert(numeric(8,2),sum(eventData14)/60) as STOPTIME,count(*) as STOPCOUNT from YQCJ_tabCycleTime ");
        cqExp.where();
        ...
        logger.debug(" 油漆车间YQCJ_EQUIPMENT_STOP查询所有【工段停线报表:YQCJ_EQUIPMENT_STOP】信息(月):  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("YQCJ_EQUIPMENT_STOP", con);
    }
...
//        cqExp.filed(null == ppdate ? null : "convert(varchar(20),PPDATE,23)", CQExp.EQ, DateUtils.format(ppdate, "yyyy-MM"));
        ...
        logger.debug(" 油漆车间YQCJ_EQUIPMENT_STOP统计所有【工段停线月报表:YQCJ_EQUIPMENT_STOP】信息:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("YQCJ_EQUIPMENT_STOP", con).get("TOTAL_NUM")));
    }
    ...
    public static List queryPmcEquipmentStopYear(Connection con, Date ppdate, java.lang.String pdLine) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select eventdata15 as PRODUCTIONLINE,eventdata16 as PRODUCTIONLINENAME,eventdata1 as STATION,convert(numeric(10,2),sum(eventData14)/60) as STOPTIME,count(*) as STOPCOUNT from YQCJ_tabCycleTime ");
        cqExp.where();
        ...
        cqExp.group("eventdata15,eventdata16,eventdata1");
        cqExp.orderByDesc("sum(eventData14)/60");
        logger.debug(" 油漆车间YQCJ_EQUIPMENT_STOP查询所有【工段停线报表:YQCJ_EQUIPMENT_STOP】信息(年):  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("YQCJ_EQUIPMENT_STOP", con);
    }
...
//        cqExp.filed(null == ppdate ? null : "convert(varchar(20),PPDATE,23)", CQExp.EQ, DateUtils.format(ppdate, "yyyy"));
        ...
        logger.debug(" 油漆车间YQCJ_EQUIPMENT_STOP统计所有【工段停线年报表:YQCJ_EQUIPMENT_STOP】信息:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("YQCJ_EQUIPMENT_STOP", con).get("TOTAL_NUM")));
    }
    ...
    public static List queryPmcEquipmentStopWeek(Connection con, Date startDate, Date endDate, java.lang.String pdLine) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select eventdata15 as PRODUCTIONLINE,eventdata16 as PRODUCTIONLINENAME,eventdata1 as STATION,convert(numeric(6,2),sum(eventData14)/60) as STOPTIME,count(*) as STOPCOUNT from YQCJ_tabCycleTime ");
        cqExp.where();
        ...
        cqExp.group("eventdata15,eventdata16,eventdata1");
        cqExp.orderByDesc("sum(eventData14)/60");
        logger.debug(" 油漆车间YQCJ_EQUIPMENT_STOP查询所有【工段停线报表:YQCJ_EQUIPMENT_STOP】信息(周):  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("YQCJ_EQUIPMENT_STOP", con);
    }
    ...
    public static List queryPmcEquipmentStopCtAgen(Connection con, Date ppdate, java.lang.String pdLine) throws Exception {
        ...
        cqExp.group("eventdata15,eventdata16,eventdata1");
        cqExp.orderByDesc("count(*)");
        logger.debug(" 油漆车间YQCJ_EQUIPMENT_STOP查询所有【工段停线报表:YQCJ_EQUIPMENT_STOP】信息(天):  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("YQCJ_EQUIPMENT_STOP", con);
    }
    ...
    public static int countPmcEquipmentStopCt(Connection con, Date ppdate, java.lang.String pdLine) throws Exception {
        ...
        logger.debug(" 油漆车间YQCJ_EQUIPMENT_STOP查询所有【工段停线报表:YQCJ_EQUIPMENT_STOP】信息:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("YQCJ_EQUIPMENT_STOP", con).get("TOTAL_NUM")));
    }
    ...
    /*public static List queryYearPmcEquipmentStopCt(Connection con, Date ppdate, java.lang.String pdLine) throws Exception {
        ...
        cqExp.orderByDesc("STOPCOUNT");
        logger.debug(" YQCJ_EQUIPMENT_STOP查询所有【工段停线报表:YQCJ_EQUIPMENT_STOP】信息:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("YQCJ_EQUIPMENT_STOP", con);
    }*/
