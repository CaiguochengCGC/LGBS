...
/**
 * 【产量报表周报表:YQCJ_PP_WEEK】的Dao操作类
 * 
 */
public class PmcPpWeekDao extends DAO {
    /**
     * 查询所有【产量报表周报表:YQCJ_PP_WEEK】信息
     * 
     * @param con
     * @return
     * @throws Exception
    ...
    public static List queryPmcPpWeek(Connection con, Date week,String banci) throws Exception {
        ...
        logger.debug("油漆车间YQCJ_PP_WEEK查询所有【产量周报表:YQCJ_PP_WEEK】信息:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("YQCJ_PP_WEEK", con);
    }

    public static int countPmcPpWeek(Connection con, Date ppdate) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select count(*) as TOTAL_NUM  from YQCJ_PP_WEEK ");
        cqExp.where();
        ...
        logger.debug("油漆车间YQCJ_PP_WEEK查询所有【产量周报表:YQCJ_PP_WEEK】信息:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("YQCJ_PP_WEEK", con).get("TOTAL_NUM")));
    }
}
