...
public class PmcPpKpidayDao extends DAO {
    ...
    public static List queryPmcPpKpiday(Connection con, Date ppdate,String banci) throws Exception {
        ...
        cqExp.select("select PPDATE, PRODUCTIONLINE,PRODUCTIONLINENAME, AVG(MTTR) AS MTTR,AVG(MTBF) AS MTBF,AVG(EQMRATE) AS EQMRATE,AVG(EQPSTOP) AS EQPSTOP, cast(SUM(STOPTIME)/60.0 as decimal(18,2)) as STOPTIME,SUM(STOPCOUNT) AS STOPCOUNT,AVG(BANCI) AS BANCI  from YQCJ_PP_KPIDAY ");
        ...
        logger.debug("油漆车间CYCJ_PP_KPIDAY查询所有【KPI日报表:YQCJ_PP_KPIDAY】信息:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("YQCJ_PP_KPIDAY", con);
    }
    
    public static List queryPmcPpKpidayPic(Connection con, Date ppdate,String banci) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select PPDATE, PRODUCTIONLINE,PRODUCTIONLINENAME, MTTR, MTBF, replace(EQMRATE,char(37),'') as EQMRATE, replace(EQPSTOP,char(37),'') as EQPSTOP, isnull(NULLIF(STOPTIME,''),0) as STOPTIME, STOPCOUNT  from YQCJ_PP_KPIDAY ");
        ...
        logger.debug("油漆车间CYCJ_PP_KPIDAY查询所有【KPI日报表:CYCJ_PP_KPIDAY】信息:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("YQCJ_PP_KPIDAY", con);
    }

    public static int countPmcPpKpiday(Connection con, Date ppdate) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select count(*) as TOTAL_NUM, STOPTIME, STOPCOUNT  from YQCJ_PP_KPIDAY ");
        ...
        logger.debug("油漆车间CYCJ_PP_KPIDAY统计【KPI日报表:CYCJ_PP_KPIDAY】信息:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("YQCJ_PP_KPIDAY", con).get("TOTAL_NUM")));
    }
}
