...
/**
 * 【工段停线报表:PMC_EQUIPMENT_STOPLINE】的Dao操作类
 * 
 */
public class PmcEquipmentStoplineDao extends DAO {
    /**
     * 查询所有【工段停线报表:PMC_EQUIPMENT_STOPLINE】信息
     * 
     * @param con
     * @return
     * @throws Exception
     ...
     * 
     */
    public static List queryPmcEquipmentStopline(Connection con, Date ppdate, Date nextPpdates,String banci,String whereSQL) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select PRODUCTIONLINE,PRODUCTIONLINENAME,BANCI,");
        cqExp.select("cast(STOPTIME/60.0 as decimal(18,2)) as STOPTIME,");
        cqExp.select("STOPCOUNT,");
        cqExp.select("cast(DATA3/60.0 as decimal(18,2)) as DATA3,");
        cqExp.select("cast(DATA4/60.0 as decimal(18,2)) as DATA4,");
        cqExp.select("cast(DATA5/60.0 as decimal(18,2)) as DATA5,");
        cqExp.select("cast(DATA6/60.0 as decimal(18,2)) as DATA6,");
        cqExp.select("cast(DATA7/60.0 as decimal(18,2)) as DATA7,");
        cqExp.select("cast(DATA11/60.0 as decimal(18,2)) as DATA11");
        cqExp.select(" from PMC_EQUIPMENT_STOPLINE ");
        cqExp.where();
cqExp.select(whereSQL);
        ...
        logger.debug(" PMC_EQUIPMENT_STOPLINE查询所有【工段停线报表:PMC_EQUIPMENT_STOPLINE】信息:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("PMC_EQUIPMENT_STOPLINE", con);
    }
    public static List queryPmcEquipmentStoplineAll(Connection con, Date ppdate, Date nextPpdates,String banci,String whereSQL) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("SELECT\n" +
                "\tPRODUCTIONLINE,\n" +
                "\tPRODUCTIONLINENAME,\n" +
                ...
                "\tSUM(cast( DATA3 / 60.0 AS DECIMAL ( 18, 2 ) )) AS DATA3,\n" +
                "\tSUM(cast( DATA4 / 60.0 AS DECIMAL ( 18, 2 ) )) AS DATA4,\n" +
                "\tSUM(cast( DATA5 / 60.0 AS DECIMAL ( 18, 2 ) )) AS DATA5,\n" +
                "\tSUM(cast( DATA6 / 60.0 AS DECIMAL ( 18, 2 ) )) AS DATA6,\n" +
                "\tSUM(cast( DATA7 / 60.0 AS DECIMAL ( 18, 2 ) )) AS DATA7,\n" +
                "\tSUM(cast( DATA11 / 60.0 AS DECIMAL ( 18, 2 ) )) AS DATA11 \n" +
                "FROM\n" +
                "\tPMC_EQUIPMENT_STOPLINE");
        cqExp.where();

        cqExp.select(whereSQL);
        cqExp.filed(null == ppdate ? null : "convert(varchar(16),PPDATE,23)", CQExp.GREATER_EQ, DateUtils.format(ppdate, DictConstants.FORMAT_DATE));
        cqExp.filed(null == ppdate ? null : "convert(varchar(16),PPDATE,23)", CQExp.LESS, DateUtils.format(nextPpdates, DictConstants.FORMAT_DATE));
        cqExp.select(" GROUP BY PRODUCTIONLINE,PRODUCTIONLINENAME");
    ...
    public static int countPmcEquipmentStopLine(Connection con, Date ppdate, Date nextPpdates) throws Exception {
        ...
        cqExp.filed(null == ppdate ? null : "convert(varchar(16),PPDATE,23)", CQExp.GREATER_EQ, DateUtils.format(ppdate, DictConstants.FORMAT_DATE));
        cqExp.filed(null == ppdate ? null : "convert(varchar(16),PPDATE,23)", CQExp.LESS, DateUtils.format(nextPpdates, DictConstants.FORMAT_DATE));
        cqExp.group(" seq,ID,PRODUCTIONLINENAME,PRODUCTIONLINE,SHIFT,OPERUSER,UPDATETIME, STOPTIME, STOPCOUNT,DATA3,DATA4,DATA5, DATA6, DATA7 ");
        cqExp.orderByAsc("seq,PRODUCTIONLINENAME");
        logger.debug(" PMC_EQUIPMENT_STOPLINE统计所有【工段停线日报表:PMC_EQUIPMENT_STOPLINE】信息:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("PMC_EQUIPMENT_STOPLINE", con).get("TOTAL_NUM")));
    }
     ...
     * @throws Exception
     */
    public static List queryMonPmcEquipmentStopline(Connection con, Date ppdate,String banci,String whereSQL) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select FPRODUCTIONLINE, PRODUCTIONLINE, PRODUCTIONLINENAME,");
        ...
        cqExp.select("(case when AVG (BANCI)=1 then '1' when AVG (BANCI)=2 then '2' else '全部' end)  as BANCI,");
        cqExp.select("cast(sum(DATA7)/60.0 as decimal(18,2)) as DATA7, ");
        cqExp.select("cast(sum(DATA11)/60.0 as decimal(18,2)) as DATA11 ");
        ...
        cqExp.filed("".equals(banci)? null : "BANCI", CQExp.EQ, banci);
        cqExp.group("seq,FPRODUCTIONLINE, PRODUCTIONLINE, PRODUCTIONLINENAME");
        cqExp.orderByAsc("seq,PRODUCTIONLINENAME");
        logger.debug(" PMC_EQUIPMENT_STOPLINE查询所有【工段停线月报表:PMC_EQUIPMENT_STOPLINE】信息:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("PMC_EQUIPMENT_STOPLINE", con);
    }
    ...
    public static int countPmcEquipmentStopLineMonth(Connection con, Date ppdate) throws Exception {
        ...
        cqExp.group(" seq,ID,PRODUCTIONLINENAME,PRODUCTIONLINE,SHIFT,OPERUSER,UPDATETIME, STOPTIME, STOPCOUNT,DATA3,DATA4,DATA5, DATA6, DATA7 ");
        cqExp.orderByAsc("seq,PRODUCTIONLINENAME");
        logger.debug(" PMC_EQUIPMENT_STOPLINE统计所有【工段停线月报表:PMC_EQUIPMENT_STOPLINE】信息:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("PMC_EQUIPMENT_STOPLINE", con).get("TOTAL_NUM")));
    }
     ...
     * @throws Exception
     */
    public static List queryYearPmcEquipmentStopline(Connection con, Date ppdate,String banci,String whereSQL) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select FPRODUCTIONLINE, PRODUCTIONLINE, PRODUCTIONLINENAME,");
        cqExp.select("cast(sum(STOPTIME)/60.0 as decimal(18,2)) as  STOPTIME,");
        cqExp.select("sum(STOPCOUNT) as STOPCOUNT,");
        ...
        cqExp.group("seq,FPRODUCTIONLINE, PRODUCTIONLINE, PRODUCTIONLINENAME");
        cqExp.orderByAsc("seq,PRODUCTIONLINENAME");
        logger.debug(" PMC_EQUIPMENT_STOPLINE查询所有【工段停线年报表:PMC_EQUIPMENT_STOPLINE】信息:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("PMC_EQUIPMENT_STOPLINE", con);
    }
    ...
    public static int countPmcEquipmentStopLineYear(Connection con, Date ppdate) throws Exception {
        ...
        cqExp.group("seq,ID,PRODUCTIONLINENAME,PRODUCTIONLINE,SHIFT,OPERUSER,UPDATETIME, STOPTIME, STOPCOUNT,DATA3,DATA4,DATA5, DATA6, DATA7 ");
        cqExp.orderByAsc("seq,PRODUCTIONLINENAME");
        logger.debug(" PMC_EQUIPMENT_STOPLINE统计所有【工段停线年报表:PMC_EQUIPMENT_STOPLINE】信息:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("PMC_EQUIPMENT_STOPLINE", con).get("TOTAL_NUM")));
    }
     ...
     * @throws Exception
     */
    public static List queryWeekPmcEquipmentStopline(Connection con, Date startDate,Date lastDate,String banci,String whereSQL) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select FPRODUCTIONLINE, PRODUCTIONLINE, PRODUCTIONLINENAME,");
        cqExp.select("cast(sum(STOPTIME)/60.0 as decimal(18,2)) as  STOPTIME,");
        cqExp.select("sum(STOPCOUNT) as STOPCOUNT,");
        ...
        cqExp.group("seq,FPRODUCTIONLINE, PRODUCTIONLINE, PRODUCTIONLINENAME");
        cqExp.orderByAsc("seq,PRODUCTIONLINENAME");
        logger.debug(" PMC_EQUIPMENT_STOPLINE查询所有【工段停线周报表:PMC_EQUIPMENT_STOPLINE】信息:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("PMC_EQUIPMENT_STOPLINE", con);
    }
    ...
    public static int countPmcEquipmentStopLineWeek(Connection con, Date startDate,Date lastDate) throws Exception {
        ...
        cqExp.group("seq,ID,PRODUCTIONLINENAME,PRODUCTIONLINE,SHIFT,OPERUSER,UPDATETIME, STOPTIME, STOPCOUNT,DATA3,DATA4,DATA5, DATA6, DATA7 ");
        cqExp.orderByAsc("seq,PRODUCTIONLINENAME");
        logger.debug(" PMC_EQUIPMENT_STOPLINE统计所有【工段停线周报表:PMC_EQUIPMENT_STOPLINE】信息:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("PMC_EQUIPMENT_STOPLINE", con).get("TOTAL_NUM")));
    }
    ...
    public static List queryPmcEquipmentStoplineKpi(Connection con, Date ppdate) throws Exception {
        ...
        cqExp.select(" from PMC_EQUIPMENT_STOPLINE A,PMC_DATE_PP B,PMC_DATE_IMPORT C");
        cqExp.where();
