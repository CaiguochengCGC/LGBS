...
/**
 * 【瓶颈设备表:PMC_DATE_PPNECK】的Dao操作类
 *
 */
public class PmcDatePpneckDao extends DAO {

	//瓶颈设备停机时间查询
	public static ComboPager queryPmcDatePpneckForStopTimeByPager(Connection con,Pager pager, Date startDate, Date endDate, int timeType, String yearType,String banci)throws Exception{

	    CQExp cqexp = CQExp.instance();

	    cqexp.select("SELECT EventData7,EventData4,EventData5,AVG(BANCI)BANCI,");
	    cqexp.select("SUM(EventData1)/60 as STOPTIME,COUNT(1) as STOPCOUNT FROM TABSTOPSTA ");
	    ...
	    logger.info("瓶颈设备停机次数表：" + cqexp.getSql());
	...
	public static int countPmcDatePpneckForStopTime(Connection con, Date ppdate)throws Exception{
	        ...
	        CQExp cqexp = CQExp.instance();
	        cqexp.select("select count(*) as TOTAL_NUM from tabStopLineCount ");
	        ...
	        logger.info("瓶颈设备停机时间表(日)：" + cqexp.getSql());
	...
	public static List queryPmcDatePpneckForStopTimeMonth(Connection con, Date ppdate)throws Exception{
        ...
        logger.info("瓶颈设备停机次数表(月)：" + cqexp.getSql());
	...
	public static int countPmcDatePpneckForStopTimeMonth(Connection con, Date ppdate)throws Exception{
        ...
        logger.info("瓶颈设备停机时间表(月)：" + cqexp.getSql());
	...
	public static List queryPmcDatePpneckForStopTimeYear(Connection con, Date ppdate)throws Exception{
        ...
        logger.info("瓶颈设备停机次数表(年)：" + cqexp.getSql());
	...
	public static int countPmcDatePpneckForStopTimeYear(Connection con, Date ppdate)throws Exception{
        ...
        logger.info("瓶颈设备停机时间表(年)：" + cqexp.getSql());

        return Integer.parseInt(String.valueOf(cqexp.getDynaBeanMap("tabStopLineCount", con).get("TOTAL_NUM")));
    }

	//瓶颈设备停机频率 查询
    public static ComboPager queryPmcDatePpneckForStopCountByPager(Connection con,Pager pager, Date startDate, Date endDate, int timeType, String yearType,String banci)throws Exception{
        ...
        logger.info("瓶颈设备停机次数频率：" + cqexp.getSql());
    ...
    public static List queryPmcDatePpneckForStopCount(Connection con, Date ppdate)throws Exception{
        ...
        logger.info("瓶颈设备停机次数表(日)：" + cqexp.getSql());
