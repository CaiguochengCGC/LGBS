...
/**
 * 【计划日期数量导入:PMC_DATE_IMPORT】的Dao操作类
 * 
 */
public class PmcDateImportDao extends DAO {
    ...
    public static List queryPmcDateImport(Connection con, Date workdate) throws Exception {
        ...
        cqExp.select("select SHIFT,WORKDATE,STARTTIME,ENDTIME, (DATEDIFF(hh,STARTTIME,ENDTIME)- RESTTIME) as LASTTIME from PMC_DATE_IMPORT");
        cqExp.where();

        cqExp.filed(null == workdate ? null : "convert(varchar(10), WORKDATE ,23)", CQExp.EQ, Tools.getTimestamp(workdate));
        logger.debug(" PMC_DATE_IMPORT计划日期数量导入:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("PMC_DATE_IMPORT", con);
    ...
    public static List queryPmcDateImportAndPp(Connection con, Date workdate) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select '生产计划时间:' as WORKDATE_TYPE, * from PMC_DATE_IMPORT");
        cqExp.where();
        ...
        logger.debug(" PMC_DATE_IMPORT计划日期数量导入AND PP:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("PMC_DATE_IMPORT", con);

    }

    /**
     * PMC_DATE_IMPORT计划日期数量导入(导入界面查询)
     * 
     * @author luoshw
     * @param con
     * @param startDate
    ...
    public static List queryPmcDateImportImport(Connection con, java.util.Date startDate, java.util.Date endDate, java.lang.String sheft) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select * from PMC_DATE_IMPORT");
        cqExp.where();
        ...
        logger.debug(" PMC_DATE_IMPORT计划日期数量导入(导入界面查询):  " + cqExp.getSql());
    ...
    public static PmcDateImportPO convertBeanMapToPO(DynaBeanMap dynaBeanMap) throws Exception {
        if (dynaBeanMap == null) {
            ...
            pmcDateImportPO.setStarttime(dynaBeanMap.get("STARTTIME") == null ? null : DateUtils.parse(String.valueOf(dynaBeanMap.get("STARTTIME")), "yyyy/MM/dd HH:mm:ss"));
            ...
            pmcDateImportPO.setResttime(dynaBeanMap.get("RESTTIME") == null ? null : Tools.getInt(dynaBeanMap.get("RESTTIME")));
    ...
    public static int countPmcDateImportImport(Connection con, Date startDate, Date endDate, String sheft) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select count(*) as  TOTAL_NUM ,convert(date, WORKDATE ,23),SHIFT from PMC_DATE_IMPORT");
        cqExp.where();
        ...
        logger.debug("统计 PMC_DATE_IMPORT计划日期数量导入(导入界面查询):  " + cqExp.getSql());

        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("PMC_DATE_IMPORT", con).get("TOTAL_NUM")));
    }
