...
/**
 * 【生产时间数据表:PMC_DATE_PP】的Dao操作类
 * 
 */
public class PmcDatePpDao extends DAO {
    /**
     * 查询【生产时间数据表:PMC_DATE_PP】信息
     * 
     * @param con
     * @return
     * @throws Exception
    ...
    public static List queryPmcDatePp(Connection con, java.util.Date ppDate) throws Exception {
        ...
        cqexp.select(" from PMC_DATE_PP A ");

        cqexp.where();
        cqexp.filed(null == ppDate ? null : "A.WORKDATE", CQExp.EQ, ppDate);
        cqexp.orderByAsc("A.PRODUCTIONLINENAME");
        logger.info("查询【生产时间数据表:PMC_DATE_PP】信息" + cqexp.getSql());

        List list = cqexp.getDynaBeanMapList("PMC_DATE_PP", con);
    ...
    public static int countPmcDatePp(Connection con, java.util.Date ppDate) throws Exception {
        ...
        logger.info("统计【生产时间数据表:PMC_DATE_PP】信息：" + cqexp.getSql());
        return Integer.parseInt(String.valueOf(cqexp.getDynaBeanMap("PMC_DATE_PP", con).get("TOTAL_NUM")));
    }
    

    /**
     * 根据时间段、工段查询【生产时间数据表:PMC_DATE_PP】信息
     * 
     * @author luoshw
     * @param con
     * @param ppDate
    ...
    /*public static List queryPmcDatePpPeriod(Connection con, java.util.Date startDate, java.util.Date endDate, java.lang.String productionLine) throws Exception {
        ...
        logger.info("根据时间段、工段查询【生产时间数据表:PMC_DATE_PP】信息：" + cqexp.getSql());
    ...
    public static List queryPmcDatePpPeriodAgen(Connection con, java.util.Date startDate, java.util.Date endDate, java.lang.String productionLine) throws Exception {
        ...
        cqexp.select("select PPDATE,PRODUCTLINE,PRODUCTLINENAME,PRODUCTTYPE,ONEWORKTIME,TWOWORKTIME,THREEWORKTIME from PMC_PP_PRODUCT_TIME");
