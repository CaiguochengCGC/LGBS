...
/**
 * 【产量年报表:ZZCJ_PP_YY】的Dao操作类
 * 
 */
public class PmcPpYyDao extends DAO {
    /**
     * 查询所有【产量年报表:ZZCJ_PP_YY】信息
     * 
     * @param con
     * @return
     * @throws Exception
    ...
    public static List queryPmcPpYy(Connection con, Date yyyy,String banci) throws Exception {
        ...
        cqExp.select("SELECT PRODUCTIONLINE,PRODUCTIONLINENAME,sum(DATA1) as DATA1,sum(DATA2) as DATA2"
        		+ ",sum(DATA3) as DATA3,sum(DATA4) as DATA4,sum(DATA5) as DATA5,sum(DATA6) as DATA6,sum(DATA7) as DATA7"
        		+ ",sum(DATA8) as DATA8,sum(DATA9) as DATA9,sum(DATA10) as DATA10,sum(DATA11) as DATA11,sum(DATA12) as DATA12"
        		+ ",AVG(YYRATE) AS YYRATE,SUM(YYREALP) AS YYREALP,SUM(YYPLANP) AS YYPLANP,AVG(BANCI) AS BANCI"
        		+ " FROM ZZCJ_PP_YY ");
        cqExp.where();
        ...
        logger.debug("总装车间ZZCJ_PP_YY查询所有【产量年报表:ZZCJ_PP_YY】信息:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("ZZCJ_PP_YY", con);
    }

    public static int countPmcPpYy(Connection con, Date yyyy) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select count(*) as TOTAL_NUM  from ZZCJ_PP_YY ");
        cqExp.where();
        ...
        logger.debug("总装车间ZZCJ_PP_YY查询所有【产量年报表:ZZCJ_PP_YY】信息:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("ZZCJ_PP_YY", con).get("TOTAL_NUM")));
    }
}
