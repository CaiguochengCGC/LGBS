...
/**
 * 【瓶颈设备表:ZZCJ_DATE_PPNECK】的Dao操作类
 * 
 */
public class PmcDatePpneckDao extends DAO {
	
	//瓶颈设备停机时间查询
	public static List queryPmcDatePpneckForStopTimeByPager(Connection con,Pager pager, Date startDate, Date endDate, int timeType, String yearType)throws Exception{
	    
	    CQExp cqexp = CQExp.instance();
	    
	    cqexp.select("SELECT top 20 b.ALARM_MESSAGE,b.DATA1,");
	    cqexp.select("SUM(DATEDIFF(ss,a.generation_time,a.timestamp))/60 as STOPTIME,COUNT(1) as STOPCOUNT FROM ZZCJ_ALARM_LOG a left join ZZCJ_tabRefer b on a.alarm_id=b.ALARMID");
	    cqexp.where("b.ALARM_MESSAGE is not null and b.ALARM_MESSAGE not like '%安灯%' and b.ALARM_MESSAGE not like '%拉绳%' and b.ALARM_MESSAGE not like '%ANDON%'");
	    cqexp.filed("a.generation_time", CQExp.GREATER_EQ, Tools.getTimestamp(startDate));	  
	    cqexp.filed("a.generation_time", CQExp.LESS, Tools.getNextDayTimestamp(endDate));
	    logger.info("####################：" + Tools.getTimestamp(startDate)+"***********"+Tools.getNextDayTimestamp(endDate));
	    //if("".equals(yearType)){
	    	cqexp.group("b.ALARM_MESSAGE,b.DATA1");
	    /*}else{
	    	cqexp.group(yearType+"convert(varchar(2),datepart(week,convert(datetime,eventdate,20))),eventdata4");
	    }*/
	    
	    cqexp.orderByDesc("SUM(DATEDIFF(ss,a.generation_time,a.timestamp))/60");
	    
	    logger.info("总装车间瓶颈故障统计分析：" + cqexp.getSql());
	...
	public static List queryPmcDatePpneckForStopTime(Connection con, Date ppdate)throws Exception{
        ...
        cqexp.select(" from ZZCJ_tabStopLineCount");
        ...
        logger.info("总装车间瓶颈设备停机次数表：" + cqexp.getSql());
	...
	public static int countPmcDatePpneckForStopTime(Connection con, Date ppdate)throws Exception{
	        ...
	        CQExp cqexp = CQExp.instance();
	        cqexp.select("select count(*) as TOTAL_NUM from ZZCJ_tabStopLineCount ");
	        ...
	        logger.info("总装车间瓶颈设备停机时间表(日)：" + cqexp.getSql());
	...
	public static List queryPmcDatePpneckForStopTimeMonth(Connection con, Date ppdate)throws Exception{
        ...
        logger.info("总装车间瓶颈设备停机次数表(月)：" + cqexp.getSql());
	...
	public static int countPmcDatePpneckForStopTimeMonth(Connection con, Date ppdate)throws Exception{
        ...
        logger.info("总装车间瓶颈设备停机时间表(月)：" + cqexp.getSql());
	...
	public static List queryPmcDatePpneckForStopTimeYear(Connection con, Date ppdate)throws Exception{
        ...
        logger.info("总装车间瓶颈设备停机次数表(年)：" + cqexp.getSql());
	...
	public static int countPmcDatePpneckForStopTimeYear(Connection con, Date ppdate)throws Exception{
        ...
        logger.info("总装车间瓶颈设备停机时间表(年)：" + cqexp.getSql());
    ...
    public static ComboPager queryPmcDatePpneckForStopCountByPager(Connection con,Pager pager, Date startDate, Date endDate, int timeType, String yearType)throws Exception{
	    ...
	    cqexp.select("SUM(EventData1)/60 as STOPTIME,COUNT(1) as STOPCOUNT FROM ZZCJ_TABSTOPSTA ");
        ...
        logger.info("总装车间瓶颈设备停机次数频率：" + cqexp.getSql());
    ...
    public static List queryPmcDatePpneckForStopCount(Connection con, Date ppdate)throws Exception{
        ...
        logger.info("总装车间瓶颈设备停机次数表(日)：" + cqexp.getSql());
