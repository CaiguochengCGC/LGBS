...
import com.hanthink.util.DictConstants;
        ...
        /**
 * 【工位CT报表:ZZCJ_STATION_CT】的Dao操作类
 * 
 */
public class PmcStationCtDao extends DAO {
	/**
	* 查询所有【工位CT报表:ZZCJ_STATION_CT】信息
	* @param con
	* @return
	* @throws Exception
	* @date 2014-03-19
	...
	*/
	...
	public static List queryDayPmcStationCt(Connection con,Date ppdate,String productionline)throws Exception{
        ...
        cqExp.select(" from ZZCJ_STATION_CT as a , ");
        cqExp.select(" (select PRODUCTIONLINE, MAX(DATA12) AS DATA12  FROM   ZZCJ_STATION_CT ");
        ...
        logger.debug("总装车间ZZCJ_STATION_CT查询所有【工位CT报表:ZZCJ_STATION_CT】信息:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("ZZCJ_STATION_CT", con);
	}
	
	public static int countDayPmcStationCt(Connection con,Date ppdate,String productionline)throws Exception{
        CQExp cqExp = CQExp.instance();
        cqExp.select("select count(*) as TOTAL_NUM from ZZCJ_STATION_CT  ");
        cqExp.where();
        ...
        logger.debug("总装车间ZZCJ_STATION_CT查询所有【工位CT报表:ZZCJ_STATION_CT】信息:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("ZZCJ_STATION_CT", con).get("TOTAL_NUM")));
    }
	...
	public static List queryMonthPmcStationCt(Connection con,Date ppdate,String productionline) throws Exception {
        ...
        logger.debug("总装车间ZZCJ_STATION_CT查询所有【工段停线报表:ZZCJ_STATION_CT】信息:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("ZZCJ_STATION_CT", con);
    }
...
//        cqExp.filed(null == ppdate ? null : "convert(varchar(20),PPDATE,23)", CQExp.EQ, DateUtils.format(ppdate, "yyyy"));
        cqExp.group("PRODUCTIONLINE, PRODUCTIONLINENAME,STATION,MODEL,DATA12,DATA4,DATA5,DATA6,DATA7,DATA8,DATA9,DATA10");
        logger.debug("总装车间ZZCJ_STATION_CT查询所有【工位CT报表:总装车间ZZCJ_STATION_CT】信息:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("ZZCJ_STATION_CT", con).get("TOTAL_NUM")));
    }
