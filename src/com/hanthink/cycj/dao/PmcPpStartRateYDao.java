...
/**
 * 【开动率年报表:CYCJ_PP_START_RATE_Y】的Dao操作类
 * 
 */
public class PmcPpStartRateYDao extends DAO {
    ...
    public static List queryPmcPpStartRateY(Connection con, Date yyyy,String banci) throws Exception {
        ...
        cqExp.select(" from CYCJ_PP_START_RATE_Y ");
        cqExp.where();
        ...
        logger.debug(" 冲压车间开动率年报表CYCJ_PP_START_RATE_Y:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("CYCJ_PP_START_RATE_Y", con);
    ...
    public static int countPmcPpStartRateY(Connection con, Date yyyy) throws Exception {
        ...
        cqExp.orderByAsc("seq,PRODUCTIONLINENAME");
        logger.debug(" 冲压车间开动率年报表CYCJ_PP_START_RATE_Y:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("CYCJ_PP_START_RATE_Y", con).get("TOTAL_NUM")));
    }
}
