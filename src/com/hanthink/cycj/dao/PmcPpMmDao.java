...
/**
 * 【产量月报表:CYCJ_PP_MM】的Dao操作类
 * 
 */
public class PmcPpMmDao extends DAO {
    /**
     * 查询所有【产量月报表:CYCJ_PP_MM】信息
     * 
     * @param con
     * @return
     * @throws Exception
    ...
    public static List queryPmcPpMm(Connection con, Date yyyyMm,String banci) throws Exception {
        ...
        logger.debug("冲压车间CYCJ_PP_MM查询所有【产量月报表:CYCJ_PP_MM】信息:  " + cqExp.getSql());
        return cqExp.getDynaBeanMapList("CYCJ_PP_MM", con);

    }

    public static int countPmcPpMm(Connection con, Date yyyyMm) throws Exception {
        CQExp cqExp = CQExp.instance();
        cqExp.select("select count(*) as TOTAL_NUM  from CYCJ_PP_MM ");
        cqExp.where();
        ...
        logger.debug("冲压车间CYCJ_PP_MM查询所有【产量月报表:CYCJ_PP_MM】信息:  " + cqExp.getSql());
        return Integer.parseInt(String.valueOf(cqExp.getDynaBeanMap("CYCJ_PP_MM", con).get("TOTAL_NUM")));
    }
}
