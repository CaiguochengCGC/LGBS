...
public class PmcSystemReportDao extends DAO {
	...
	public static List QueryPmcSystemReport(Connection con, String workdate,String banci)throws Exception {
		...
		cqexp.select(" FROM CYCJ_EQUIPMENT_STOPLINE A  ");
		cqexp.select(" LEFT JOIN CYCJ_PP_PRODUCT_TIME B ");
		...
		cqexp.select(" LEFT JOIN CYCJ_PP_START_RATE C ");
		...
		cqexp.select(" LEFT JOIN (select SUM(productTotal) as productTotal,SUM(BP32) as BP32,EventData5,BANCI,PPDATE FROM CYCJ_PP_PRODUCT group by EventData5,BANCI,PPDATE) D ");
		...
		logger.info("冲压车间查询【冲压车间总报表】信息：" + cqexp.getSql());
		
		List list = cqexp.getDynaBeanMapList("CYCJ_SYSTEM_REPORT", con);
	...
	public static List getProductActNum(Connection con, String production, String workdate) throws Exception {
		CQExp cqExp = CQExp.instance();
		cqExp.select("SELECT COUNT(EVENTDATA4) AS EVENTDATA4 FROM CYCJ_tabProduct");
		...
		logger.info("冲压车间查询【冲压车间总报表】计算实际产量：" + cqExp.getSql());
		return cqExp.getDynaBeanMapList("CYCJ_SYSTEM_REPORT", con);
	}
	...
	public static List getProductPlanNum(Connection con, String production, String workdate) throws Exception {
		CQExp cqExp = CQExp.instance();
		cqExp.select("SELECT SUM(CONVERT(INT,ISNULL(EventDate31,0))) AS EventDate31 FROM CYCJ_tabProductHour");
		cqExp.where();
		...
		logger.info("查询【冲压车间总报表】计划总产量：" + cqExp.getSql());
		return cqExp.getDynaBeanMapList("CYCJ_SYSTEM_REPORT", con);
	}
	
	 public static int countPmcSystemReport(Connection con, java.util.Date workdate) throws Exception {
	    CQExp cqexp = CQExp.instance();
	    cqexp.select("select count(*) as TOTAL_NUM  from CYCJ_EQUIPMENT_STOPLINE a  , CYCJ_tabProductHour b ,CYCJ_PP_START_RATE_DAY c where a.PRODUCTIONLINE = b.EventDate1 ");
		...
		logger.info("查询【冲压车间总报表】总数：" + cqexp.getSql());

	    return Integer.parseInt(String.valueOf(cqexp.getDynaBeanMap("CYCJ_SYSTEM_REPORT", con).get("TOTAL_NUM")));
	 ...
	 public static List getRestTime(Connection con,String workdate) throws Exception {
		...
		logger.info("查询【冲压车间总报表】休息时间：" + cqexp.getSql());
