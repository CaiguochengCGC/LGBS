...
Ext.ux.form.LovCombo = Ext.extend(Ext.form.ComboBox, {
			 ...
			 *      state. It is automatically added to existing fields.
			 *      (defaults to "checked" - change it only if it collides with
			 *      your normal field)
			 */
			checkField : 'checked'
			...
			// {{{
			/**
			 * Sets the value of the LovCombo. The passed value can by a falsie
			 * (null, false, empty string), in which case the combo value is
			 * cleared or separator separated string of values, e.g. '3,5,6'.
			 * 
			 * @param {Mixed}
			 *            v value
			 */
				...
				if (this.forceSelection) {
					if (val.length > 0 && val != this.emptyText) {
						this.el.dom.value = Ext
								.isDefined(this.lastSelectionText)
					...
					for (var i = 0; i < texts.length; i++) {
						var rec = this.findRecord(this.displayField, texts[i]
										.trim());
						if (rec) {
