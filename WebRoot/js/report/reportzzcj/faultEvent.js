...
(function() {
    ...
    var sectionStore = new Ext.data.JsonStore({
        ...
        root : 'root.PMC_PP_STATION.rs',
        baseParams : {
            action : 'ZZCJ_QUERY_PMC_PP_STATION_ACTION',
...
//            CODE_TYPE : 'EventData16'
  ...
  //属性
    var attrStore = new Ext.data.JsonStore({
        ...
        root : 'root.PMC_PP_STATION.rs',
        baseParams : {
            action : 'ZZCJ_QUERY_PMC_PP_ATTR_ACTION',
//            action : 'QUERY_PMC_PP_STATION_CT_ALL_ACTION',
//            CODE_TYPE : 'EventData16'
            CODE_TYPE : 'DATA3'
        },
...
//					value : new Date(new Date().getTime() - 4 * 24 * 60 * 60 * 1000),
                ...
                listeners : {
                    beforeQuery : function(c){
                        delete c.combo.lastQuery;
                        var gd = faultEventPanel.findQueryCompment('DATA3').getValue();
                        attrStore.baseParams['QURY_TYPE'] = gd;
                        attrStore.load();
                    },
                    select : function(c,r,index){
	                   // faultEventPanel.findQueryCompment('attr').setValue(null);
	                }
                    
                }
            }],
            action : 'ZZCJ_QUERY_ALARM_LOG_ACTION',
        ...
        gridConfig : {
            ...
            storeFields : ['PRODUCTIONLINENAME','WORKTIME','GENERATION_TIME','TIMESTAMP','BANCI','ALARM_MESSAGE','RESOURCE','DATA1','DATA2','DATA3','DATA7'],
            ctTopbarComponts : [{ 
                iconCls : 'excel',
                text : '导出',
    ...
    var exportFn = function(){
        ...
        var url = 'json?action=ZZCJ_EXPORT_ALARM_LOG_ACTION';
